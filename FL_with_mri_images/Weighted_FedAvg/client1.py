...
adni2 = available_datasets.load_ADNI2_data(x_range_from = 32, x_range_to = 161,
                                           ...
                                           flavor = flavor)
adni2_images = adni2['images']
adni2_labels = adni2['labels']
adni2_labels = to_categorical(adni2_labels)

# Split the data into training and testing set
X_train, X_test, Y_train, Y_test = train_test_split(adni2_images, adni2_labels, test_size=0.2, random_state=42)
...
class ADNI2_Client(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        _class_weights = util.get_class_weights(self.y_train, self.y_train.shape[0])
...
model = util.DenseNet(adni2_images.shape[1:], adni2_labels.shape[1])

# Start Flower client
client = ADNI2_Client(model, X_train, Y_train, X_test, Y_test)
