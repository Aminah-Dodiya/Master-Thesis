...
aibl = available_datasets.load_AIBL_data(x_range_from = 32, x_range_to = 161,
                                         ...
                                         flavor = flavor)
aibl_images = aibl['images']
aibl_labels = aibl['labels']
aibl_labels = to_categorical(aibl_labels)

# Split the data into training and testing set
X_train, X_test, Y_train, Y_test = train_test_split(aibl_images, aibl_labels, test_size=0.2, random_state=42)
...
class AIBL_Client(fl.client.NumPyClient):
...
model = util.DenseNet(aibl_images.shape[1:], aibl_labels.shape[1])

# Start Flower client
client = AIBL_Client(model, X_train, Y_train, X_test, Y_test)
